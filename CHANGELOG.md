...
### Changed
- **Mobile App (BETA)** - Finalized iOS app for initial release
  - Server settings now integrated as section in Settings page (better UX)
  - Added "Verify Connection" button to test server reachability and API key validity
  - Added app icon and splash screens (light/dark mode)
  - Improved safe area support for iOS notch/Dynamic Island
  - Simplified server settings to display-only (disconnect to reconfigure)
